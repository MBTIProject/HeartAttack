...
public interface SurveyRepository extends JpaRepository <Survey, Long>, SurveyRepositoryCustom {
    @Query("select distinct s from Survey s left join fetch s.poster where s.poster.id=:poster_id")
    List<Survey> findByPoster_id(@Param("poster_id") Long poster_id);

    @Query("select s from Survey s left join fetch s.poster where s.poster.id = :posterId and s.id = :surveyId")
